...
export class SystemsController {
  ...
  listSystems(@QueryParams('page') page: number, @QueryParams('limit') limit: number): GetSystems200Response {
    const systems = slicePage(Object.values(universe.systems), page, limit).map(system => {
  ...
  market(@PathParams('systemSymbol') systemSymbol: string, @PathParams('waypointSymbol') waypointSymbol: string, @Context('auth') context: AuthToken): GetMarket200Response {
    ...
    let hasShip = false
    if (context) {
      hasShip = universe.ships.find(ship => ship.agentSymbol == context.identifier && ship.navigation.current.symbol === waypointSymbol) !== undefined
    }
  ...
  jumpGate(@PathParams('systemSymbol') systemSymbol: string, @PathParams('waypointSymbol') waypointSymbol: string, @Context('auth') context: AuthToken): GetJumpGate200Response {
    ...
    return {
      data: {
        ...
        connectedSystems: Object.values(universe.systems).filter(s => {
