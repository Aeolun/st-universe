...
import {
  ...
  DeliverContract200Response, DeliverContractRequest, FulfillContract200Response,
...
export class ContractsController {
  @Get("/contracts")
  listShips(@AgentParam() agent: Agent, @QueryParams('page') page: number = 1, @QueryParams('limit') limit: number = 10): GetContracts200Response {
    return {
      data: slicePage<Contract>(agent.contracts, page, limit).map(contract => {
  ...
  get(@AgentParam() agent: Agent, @PathParams('contractId') contractId: string): GetContract200Response {
    const contract = agent.contracts.find(contract => contract.symbol === contractId)
  ...
  accept(@AgentParam() agent: Agent, @PathParams('contractId') contractId: string): AcceptContract200Response {
    const contract = agent.contracts.find(contract => contract.symbol === contractId)
  ...
  deliver(@AgentParam() agent: Agent, @PathParams('contractId') contractId: string, @BodyParams() body: DeliverContractRequest): DeliverContract200Response {
    const contract = agent.contracts.find(contract => contract.symbol === contractId)
    ...
    if (!ship.cargo[body.tradeSymbol]) throw new Error(`Ship ${body.shipSymbol} has no ${body.tradeSymbol} in cargo`)

    for(const term of contract.terms) {
      if (term.type === 'PROCUREMENT' && term.deliveryGoodSymbol === body.tradeSymbol) {
        if (ship.navigation.current.symbol !== term.deliveryWaypointSymbol) {
          throw new Error(`Ship ${body.shipSymbol} is not at waypoint ${term.deliveryWaypointSymbol}`)
        }

        term.deliveredQuantity += Math.min(ship.cargo[body.tradeSymbol]!, body.units)
        ship.cargo[body.tradeSymbol]! -= Math.min(ship.cargo[body.tradeSymbol]!, body.units)
      }
    }
  ...
  fulfill(@AgentParam() agent: Agent, @PathParams('contractId') contractId: string): FulfillContract200Response {
    const contract = agent.contracts.find(contract => contract.symbol === contractId)
