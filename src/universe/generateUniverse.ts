...
export async function generateUniverse() {
        ...
        for(let step = 1; step < STEPS_PER_ARM; step++) {
            ...
            for(let j = 0; j < systemsInStep; j++) {
                ...
                do {
                    ...
                    (
                        universe.systems.some(system => getDistance(system, {x: potentialX, y: potentialY}) < MINIMUM_DISTANCE_APART) ||
                        universe.systems.every(system => getDistance(system, {x: potentialX, y: potentialY}) > MAXIMUM_DISTANCE_APART)
                    )
                    && attempts < 30)
                if (attempts >= 30) {
    ...
    for(const faction of factionNames) {
        ...
        let attempts = 0, systemsInInfluence = 0
        do {
            factionSystem = pickRandom(populationCenters)
            systemsInInfluence = 0
            for (const system of universe.systems) {
                if (getDistance(system, factionSystem) < FACTION_INNER_INFLUENCE_RADIUS) {
                    systemsInInfluence++
                }
            }
    ...
    for(const faction of factionNames) {
        ...
        for (const system of universe.systems) {
            ...
            } else if (distance < FACTION_OUTER_INFLUENCE_RADIUS && system.factions.length <= 0) {
                if (percentageChance(30+((FACTION_OUTER_INFLUENCE_RADIUS-distance)/FACTION_OUTER_INFLUENCE_RADIUS)*40)) {
                    system.waypoints.forEach(wp => wp.ownedBy = faction)
                    system.factions = [faction]
                }
            }
    ...
    for(let i = 0; i < 3; i++) {
        ...
        for(const faction of randomizedFactionOrder) {
            ...
            for (const system of universe.systems) {
                ...
                if (distance < FACTION_OUTER_INFLUENCE_RADIUS && system.factions.length <= 0) {
                    if (percentageChance(30+((FACTION_OUTER_INFLUENCE_RADIUS-distance)/FACTION_OUTER_INFLUENCE_RADIUS)*40)) {
                        system.waypoints.forEach(wp => wp.ownedBy = faction)
                        system.factions = [faction]
                    }
    ...
    universe.systems.forEach(system => {
        let minDist = 10000
        universe.systems.forEach(otherSystem => {
            if (system === otherSystem) return;
            const dist = getDistance(system, otherSystem)
            if (dist < minDist) minDist = dist
        })

        totalMin += minDist / universe.systems.length
    ...
    console.log(`Universe with ${universe.systems.length} systems, ${totalWaypoints} waypoints generated.`)
    ...
    fs.writeFileSync('./systems.json', JSON.stringify(universe.systems.slice(0, 100), null, 2))
...
function renderFactionControl(system: System, context: CanvasRenderingContext2D) {
    context.beginPath()
    context.arc(CANVAS_SIZE / 2 + Math.round(system.x * scale), CANVAS_SIZE / 2 + Math.round(system.y * scale), 2, 0, 2 * Math.PI, false);
