...
waypointTraitNames.forEach((trait) => {
  ...
  if (traitData.extractableResources) {
    traitData.extractableResources.forEach((tg) => {
      tradeGoodsGenerated.add(tg.tradegood);
    });
  }
...
industryNames.forEach((industry) => {
  ...
  Object.keys(industryData.exports ?? {}).forEach((tg: TradeGood) => {
    ...
    if (!tradeGoodData) {
      console.log(
        `Industry ${industry} exports ${tg} but trade good is not defined.`
      );
    }
    if ("components" in tradeGoodData) {
      if (
        !Object.keys(industryData.imports ?? {}).every((tg) =>
          Object.keys(tradeGoodData.components).includes(tg)
        )
      ) {
        console.log(
          `Industry ${industry} exports ${tg} but not all components are defined for trade good.`
        );
      }
    } else {
      console.log(
        `Industry ${industry} constructs ${tg} from ${Object.keys(
          industryData.imports ?? {}
        ).join(", ")}, but same components are not defined for trade good.`
      );
    }
  });
});
